...
drop(void)
{
	...
	*new = *obj;
    ...
    get_desc(obj, desc + utf8strlen(mesg[92]), 0);
#endif /* not JAPAN */
    message(desc, 0);
    (void) reg_move();
}
...
take_off(void)
{
	...
	if (rogue.armor->is_cursed) {
	    ...
	    get_desc(obj, desc + utf8strlen(mesg[94]), 0);
#endif /* not JAPAN */
	    message(desc, 0);
	    print_stats(STAT_ARMOR);
	    (void) reg_move();
...
wear(void)
{
    ...
    get_desc(obj, desc + utf8strlen(mesg[100]), 0);
#endif /* not JAPAN */
    message(desc, 0);
    do_wear(obj);
    print_stats(STAT_ARMOR);
...
wield(void)
{
	...
	get_desc(obj, desc + utf8strlen(mesg[107]), 0);
#endif /* not JAPAN */
	message(desc, 0);
	do_wield(obj);
	(void) reg_move();
